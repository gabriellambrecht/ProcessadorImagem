...
public class ImageAnalizer extends Application {
    ...
    private void initComponentsMenu() {
        ...
        informacoesE.setOnAction((ActionEvent event) -> {
        	loadHistogramer(new Histograma(new Image(reader)).getHistogramaQuadrante(1));
        });
        ...
        alteracoesA.setOnAction((ActionEvent event) -> {
        	AlteraImagem(new pdi.Alteracoes(new Image(reader)).setBrancoMaiorMediaQuadrante(2));
        });
        MenuItem alteracoesB = new MenuItem("Valores maiores ou iguais a moda do quadrante 4 recebem 200");
        alteracoesB.setOnAction((ActionEvent event) -> {
        	AlteraImagem(new pdi.Alteracoes(new Image(reader)).setValorMaiorModaQuadrante(4, 200));
        });
        MenuItem alteracoesC = new MenuItem("Valores maiores ou iguais a mediana do quadrante 3 recebem 220");
        alteracoesC.setOnAction((ActionEvent event) -> {
        	AlteraImagem(new pdi.Alteracoes(new Image(reader)).setValorMaiorMedianaQuadrante(3, 220));
        });
        MenuItem alteracoesD = new MenuItem("Valores menores que a média do quadrante 2 recebem 100");
        alteracoesD.setOnAction((ActionEvent event) -> {
          	AlteraImagem(new pdi.Alteracoes(new Image(reader)).setValorMenorMediaQuadrante(2,100));
        });
        ...
        alteracoesE.setOnAction((ActionEvent event) -> {
            Image imagem = new pdi.Alteracoes(new Image(reader)).setValorMaiorMediaQuadrante(2,0);
            AlteraImagem(new pdi.Alteracoes(imagem).setValorMenorMedianaQuadrante(3,255));
        });
        ...
        transformacaoRotacao.setOnAction((ActionEvent event) -> {
            ...
            result.ifPresent(angulo -> AlteraImagem(new pdi.Transformacoes(new Image(reader)).rotacaoAngulo(Integer.valueOf(angulo))));
        });
        MenuItem transformacaoRotacao2 = new MenuItem("Rotação deslizante");
        transformacaoRotacao2.setOnAction((ActionEvent event) -> {
        	executaRotacao();
        });
        ...
        transformacaoAmpliacao.setOnAction((ActionEvent event) -> {
            ...
            Optional<String> result = dialog.showAndWait();
            result.ifPresent(fator -> AlteraImagem(new pdi.Transformacoes(new Image(reader)).amplia(Double.valueOf(fator))));
        });
        ...
        transformacaoReducao.setOnAction((ActionEvent event) -> {
            ...
            dialog.setContentText("Fator:");
            Optional<String> result = dialog.showAndWait();
            result.ifPresent(fator -> AlteraImagem(new pdi.Transformacoes(new Image(reader)).reduz(Double.valueOf(fator))));
        });
        Menu transformacaoEspalhamento = new Menu("Espelhamento");
        MenuItem transformacaoEspalhamentoHor = new MenuItem("Horizontal");
        transformacaoEspalhamentoHor.setOnAction((ActionEvent event) -> {
			AlteraImagem(new pdi.Transformacoes(new Image(reader)).espelhaHorizontal());
        });
        MenuItem transformacaoEspalhamentoVer = new MenuItem("Vertical");
        transformacaoEspalhamentoVer.setOnAction((ActionEvent event) -> {
			AlteraImagem(new pdi.Transformacoes(new Image(reader)).espelhaVertical());
        });
        ...
        filtroMediana.setOnAction((ActionEvent event) -> {
            AlteraImagem(new pdi.Filtros(new Image(reader)).filtroMediana());
        });
        MenuItem filtroGauss = new MenuItem("Gauss");
        filtroGauss.setOnAction((ActionEvent event) -> {
        	AlteraImagem(new pdi.Filtros(new Image(reader)).filtroGauss());
        });
        ...
        extracaoQuadrado.setOnAction((ActionEvent event) -> {
            ...
            alert.setHeaderText(null);
            alert.setContentText(new pdi.Extrator(new Image(reader)).extraiQuadrado());
            alert.showAndWait();
        });
        ...
        extracaoMenorCirculo.setOnAction((ActionEvent event) -> {
            ...
            alert.setTitle("Informações");
            alert.setHeaderText(null);
            alert.setContentText(new pdi.Extrator(new Image(reader)).extraiMaiorCirculo());
            alert.showAndWait();
        });
        ...
        extracaoTodosCirculo.setOnAction((ActionEvent event) -> {
            ...
            alert.setTitle("Informações");
            alert.setHeaderText(null);
            alert.setContentText(new pdi.Extrator(new Image(reader)).extraiCirculos());
            alert.showAndWait();
        });
    ...
    private void executaThreshold() {
        ...
        th.addObserver((threshold) -> {
        	AlteraImagem(new pdi.Alteracoes(new Image(reader)).threshold(threshold));
        });
        th.setValue(150);
    ...
    private void executeDeteccaoPrewitt(){
    	 ControleDeslizante th = new ControleDeslizante(0,255);
         th.addObserver((threshold) -> {
         	AlteraImagem(new pdi.Filtros(new Image(reader)).deteccaoPrewitt(threshold));
         });
         th.setValue(150);
         Dialog<Double[]> dialog = new Dialog<>();
         dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
    ...
    private void executeDeteccaoCanny(){
   	 ControleDeslizante th = new ControleDeslizante(0,255);
        th.addObserver((threshold) -> {
        	AlteraImagem(new pdi.Filtros(new Image(reader)).deteccaoCanny(threshold));
        });
        th.setValue(150);
        Dialog<Double[]> dialog = new Dialog<>();
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
    ...
    private void executeDeteccaoLaplaciano(){
   	 ControleDeslizante th = new ControleDeslizante(0,255);
        th.addObserver((threshold) -> {
        	AlteraImagem(new pdi.Filtros(new Image(reader)).deteccaoLaplaciano(threshold));
        });
        th.setValue(150);
        Dialog<Double[]> dialog = new Dialog<>();
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
    ...
    private void executaRotacao() {
        ...
        th.addObserver((angulo) -> {
        	AlteraImagem(new pdi.Transformacoes(new Image(reader)).rotacaoAngulo(angulo));
        });
        th.setValue(0);
        Dialog<Double[]> dialog = new Dialog<>();
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
