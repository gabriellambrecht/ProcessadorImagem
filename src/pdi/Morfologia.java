...
public class Morfologia {
    ...
    public boolean testeHolt(int x, int y, boolean primeiro) {
        ...
        //Se for primeiro
        if(primeiro){
        	if(!edgeC || ( v(imagem.L(x,y)) && v(imagem.S(x,y)) && ( v(imagem.N(x,y)) || v(imagem.O(x,y))))){
        		return true;
        	} else {
        		return false;
        	}
        } else {
        	if( !edgeC || ( v(imagem.O(x,y)) && v(imagem.N(x,y)) && ( v(imagem.S(x,y)) || v(imagem.L(x,y)) ))){
        		return true;
        	} else{
        		return false;
        	}
    ...
    public boolean testeHoltJuntas(int x, int y) {
        ...
        if (v(imagem.getPixel(x,y)) && (!edgeC || (edgeL && v(imagem.N(x,y)) && v(imagem.S(x,y))) 
